...
var PIX_PRODUCTS = {
  seguro: { price: 19.48, name: "Seguro Prestamista - SuperSim" },
  ...
  up6:  { price: 17.02, name: "Taxa de Registro do Contrato" },
  ...
  // ── DOWNSELL (50% OFF) ──
  seguro_ds: { price: 9.74, name: "Seguro Prestamista - SuperSim" },
...
function _pixFirePurchase() {
  ...
  var prod = PIX_PRODUCTS[_pixCurrentUpKey];
  var purchaseData = {
    value: _pixCurrentAmount || (prod ? prod.price : 0),
    currency: 'BRL',
    content_name: prod ? prod.name : _pixCurrentUpKey,
    content_type: 'product'
  };

  // Purchase principal — com eventID = txnId para dedup browser↔CAPI
  if (typeof fbq === 'function') {
    fbq('track', 'Purchase', purchaseData, { eventID: _pixCurrentTxn });
  }

  // Notifica Utmify do Purchase
  try {
    if (window.Utmify && typeof window.Utmify.trackPurchase === 'function') {
      window.Utmify.trackPurchase(purchaseData.value);
    }
  } catch(e) {}
  ...
  if (base === 'seguro') {
    if (typeof fbq === 'function') {
      fbq('trackCustom', 'FrontSale', purchaseData, { eventID: 'fs_' + _pixCurrentTxn });
    }
  }
}
...
function _pixOpenModal(upKey) {
  ...
  .then(function(data) {
    ...
    try{var _q=qrcode(0,'M');_q.addData(data.qrcode);_q.make();document.getElementById('pix-qr-img').src=_q.createDataURL(5,4);}catch(_e){document.getElementById('pix-qr-img').src='https://api.qrserver.com/v1/create-qr-code/?data='+encodeURIComponent(data.qrcode)+'&size=188x188&margin=6&bgcolor=ffffff';}
